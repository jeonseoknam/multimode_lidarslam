...
bool SlamComponent::reserveDebugCloudDumpFrame(int * frame_index)
{
...
void SlamComponent::dumpDebugCloudStage(
...
bool SlamComponent::initializeMap(const pcl::PointCloud <pcl::PointXYZI>::Ptr & tmp_ptr, const std_msgs::msg::Header & header)
{
...
bool SlamComponent::refreshRegistrationTargetFromTargetedCloud()
{
  pcl::PointCloud<pcl::PointXYZI>::Ptr targeted_cloud_ptr;
  {
    std::lock_guard<std::mutex> lock(mtx_);
...
const char * SlamComponent::trackingStateName(TrackingState state) const
{
  switch (state) {
